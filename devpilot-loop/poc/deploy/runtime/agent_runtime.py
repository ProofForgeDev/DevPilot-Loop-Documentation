"""
[PoC 实现层]AgentRuntime — FastAPI 实现 AgentTeams 兼容的消息传递
============================================================
本模块使用 FastAPI 实现 AgentTeams（HiClaw）兼容接口，提供进程内状态管理和消息路由。
的消息路由、任务派发与结果回传，基于 AgentTeams 规范实现。
- 消息格式、状态机转换、上下文（trace_id）传递结构均按 AgentTeams 规范设计
- 半决赛前需替换为 agentteams SDK 原生调用，预计改动量 < 200 行
已实现 AgentTeams 兼容接口（半决赛 P0 任务）
===============================================
...
app = create_app()  # PoC 实现层：FastAPI 替代 AgentTeams SDK


if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=int(os.environ.get("PORT", 8001)))
from metrics import inc, get_metrics_text\n
